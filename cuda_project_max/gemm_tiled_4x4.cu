...
// ==========================================
// RTX 4050 (AD107) Tuned Parameters
// ==========================================
#define BM 128
#define BN 128
#define BK 8    
...
// Kernel: gemm_tiled_4x4_register

__launch_bounds__(256, 2)
__global__ void gemm_tiled_4x4_register(float * __restrict__ A, float * __restrict__ B, float * __restrict__ C, int M, int N, int K) {
...
int main(int argc, char const *argv[]) {
    ...
    // Warmup
    gemm_tiled_4x4_register<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
    cudaDeviceSynchronize();

    // Record
    cudaEventRecord(start);
    for(int i=0; i<5; i++) { // Run 5 times to get stable average
        gemm_tiled_4x4_register<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
    }
    ...
    milliseconds /= 5.0f; // Average time

    // Copy back
    CHECK(cudaMemcpy(h_C, d_C, size_C, cudaMemcpyDeviceToHost));

    // Verify (Check corner and center)
    printf("Verifying...\n");
    bool correct = true;
    ...
    printf("Note: RTX 4050 Theoretical Peak (FP32) is approx 9000 GFLOPS.\n");
